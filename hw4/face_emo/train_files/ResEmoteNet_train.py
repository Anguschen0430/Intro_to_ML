...
# Load the dataset
train_dataset = Four4All(csv_file='../rafdb_large/train_labels.csv',
                         img_dir='../rafdb_large/train', transform=transform_train)
train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)
train_image, train_label = next(iter(train_loader))


val_dataset = Four4All(csv_file='../rafdb_large/val_labels.csv', 
                       img_dir='../rafdb_large/val', transform=transform)
...
optimizer = optim.SGD(model.parameters(), lr=0.0022, momentum=0.9, weight_decay=1e-4)
...
for epoch in range(num_epochs):
    ...
    if val_acc > best_val_acc:
        ...
        patience_counter = 0 
        torch.save(model.state_dict(), f'checkpoints/res/res_{epoch}_{val_acc:.2f}.pth')
    else:
        patience_counter += 1
        print(f"No improvement in validation accuracy for {patience_counter} epochs.")
    
    if patience_counter > patience and train_acc > 0.85:
